...
def create_order(order_data: CreateOrderRequest):
    ...
    order_number = f"RST-{now.strftime('%Y')}-{str(rst_count + 1).zfill(4)}"
    ...
    new_order = {
        ...
        "expected_delivery": (now + timedelta(days=order_data.lead_time_days)).isoformat(),
